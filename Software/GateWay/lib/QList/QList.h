...
void QList<T>::pop_front()
{
	if(start!=NULL)
	{
		...
		start->prev = NULL;
		else // QList became empty so we need to clear end
		end = NULL;
		delete tmp;
...
void QList<T>::pop_back()
{
	if(end!=NULL)
	{
		...
		else // QList became empty so we need to clear start
		start = NULL;
		delete tmp;
		len--; // Decrease counter
	}
...
T QList<T>::front()
{
	if(start!=NULL) return start->item;
